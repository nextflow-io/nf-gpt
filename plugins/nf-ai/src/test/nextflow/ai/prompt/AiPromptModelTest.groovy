 ...
 */
class AiPromptModelTest extends Specification {

    def 'should render schema' () {
        expect:
        AiPromptModel.renderSchema([foo:'string']) == 'You must answer strictly in the following JSON format: {"result": [{"foo": (type: string)}] }'
    }

    def 'should render schema /1' () {
        expect:
        AiPromptModel.schema0([foo:'string']) == '{"foo": (type: string)}'
    }

    def 'should render schema /2' () {
        expect:
        AiPromptModel.schema0(SCHEMA) == EXPECTED
    ...
    def 'should render json response' () {
        ...
        def model = new AiPromptModel(session).build()
    ...
    def 'should decode response to a list' () {
        ...
        result = AiPromptModel.decodeResponse0(resp, SCHEMA)
        then:
    ...
    def 'should check it is an index map' () {
        ...
        expect:
        AiPromptModel.isIndexMap([0: [a: 'this', b:'that'], 1: [a: 'foo', b:'bar']], SCHEMA)
        AiPromptModel.isIndexMap(['0': [a: 'this', b:'that'], '1': [a: 'foo', b:'bar']], SCHEMA)
        !AiPromptModel.isIndexMap(['x': [a: 'this', b:'that'], 'y': [a: 'foo', b:'bar']], SCHEMA)
    }
