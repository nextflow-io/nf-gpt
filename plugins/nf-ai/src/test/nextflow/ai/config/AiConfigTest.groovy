 ...
 */
class AiConfigTest extends Specification {

    def 'should create default config' () {
        when:
        def config = new AiConfig([:], [OPENAI_API_KEY:'my-api-key'])
        then:
    ...
    def 'should create config with custom opts' () {
        when:
        def config = new AiConfig([endpoint:'http://foo.com', model:'gpt-5', apiKey: 'xyz'], [OPENAI_API_KEY:'my-api-key'])
        then:
    ...
    def 'should create from session' () {
        ...
        when:
        def config = AiConfig.config(session)
        then:
