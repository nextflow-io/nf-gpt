...
import nextflow.ai.config.AiConfig
...
class AiPromptModel {
    ...
    AiPromptModel(Session session) {
        this.config = AiConfig.config(session)
    }

    AiPromptModel withModel(String model) {
        this.model = model
        return this
    }

    AiPromptModel withDebug(Boolean value) {
        this.debug = value
        return this
    }
